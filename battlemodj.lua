...
_addon.version = '3.31-j2'
_addon.name = 'BattleModJ'
_addon.author = 'Byrth, maintainer: SnickySnacks, Japanese localization: DB'
...
windower.register_event('load',function()
    ...
    -- action_messages はenしか定義されていないので一旦jaにenをコピー
    for k, v in pairs(res.action_messages) do
        res.action_messages[k].ja = v.en
    end

    -- action_messages_ja に個別に定義した日本語メッセージを上書き
    res_messages_ja = require('action_messages_ja')
    for k, v in pairs(res_messages_ja) do
        res.action_messages[k].ja = v.ja
    end
...
------- ACTION MESSAGE -------
    elseif id == 0x29 then
        ...
        elseif am.message_id == 800 then -- Spirit bond message
            local status = color_it(windower.to_shift_jis(res.buffs[am.param_1][language]),color_arr.statuscol)
            ...
            if simplify then
                ...
                local msg = res.action_messages[am.message_id]['en']
        ...
        elseif am.message_id == 206 and condensetargets then -- Wears off messages
            ...
            if enfeebling:contains(am.param_1) and res.buffs[param_1] then
                status = color_it(windower.to_shift_jis(res.buffs[param_1][language]),color_arr.enfeebcol)
            elseif color_arr.statuscol == rcol then
                status = color_it(windower.to_shift_jis(res.buffs[am.param_1][language]),string.char(0x1F,191))
            else
                status = color_it(windower.to_shift_jis(res.buffs[am.param_1][language]),color_arr.statuscol)
            end
        ...
        elseif passed_messages:contains(am.message_id) then
            ...
            if fields.item then
                ...
                item = nf(res.items[am.param_1],language)
            end
            ...
            if fields.skill and res.skills[am.param_1] then
                skill = windower.to_shift_jis(res.skills[am.param_1][language]:lower())
            end
            ...
            outstr = (clean_msg(outstr
                :gsub('${actor}\'s',color_it(actor.name or '',color_arr[actor.owner or actor.type])..'\'s'..actor.owner_name)
                :gsub('${actor}',color_it(actor.name or '',color_arr[actor.owner or actor.type])..actor.owner_name)
                :gsub('${status}',status or '')
                :gsub('${item}',color_it(item or '',color_arr.itemcol))
                :gsub('${target}\'s',color_it(target.name or '',color_arr[target.owner or target.type])..'\'s'..target.owner_name)
                :gsub('${target}',color_it(target.name or '',color_arr[target.owner or target.type])..target.owner_name)
            ...
            windower.add_to_chat(res.action_messages[am.message_id]['color'],windower.to_shift_jis(outstr))
            am.message_id = false
        elseif am.message_id == 277 then

        elseif am.message_id == 283 then -- 効果がなかった
        ...
        -- 202 is the Time Remaining message, which (interestingly) uses all the number params.
            print('debug_EAM#'..am.message_id..': '..res.action_messages[am.message_id]['en']..' '..am.param_1..'   '..am.param_2..'   '..am.param_3)
...
------------ SYNTHESIS ANIMATION --------------
    ...
    elseif id == 0x01B then
        filterload(windower.to_shift_jis(res.jobs[original:byte(9)][language..'s']))
    end
end)
...
function multi_packet(...)
    ...
    outstr = clean_msg(windower.to_shift_jis(outstr)
