...
--    act.actor.name = act.actor and act.actor.name and string.gsub(act.actor.name,'[- ]', {['-'] = string.char(0x81,0x7C), [' '] = string.char(0x81,0x3F)}) --fix for ffxi chat splits on trusts with - and spaces
    act.actor.name = act.actor and act.actor.name
    ...
    for i,v in pairs(act.targets) do
        for n,m in pairs(v.actions) do
            if m.message ~= 0 and res.action_messages[m.message] ~= nil then
                ...
                if m.reaction == 11 and act.category == 1 then m.simp_name = '受け流し'
                ...
                elseif m.message == 15 then m.simp_name = 'ミス'
                elseif m.message == 29 or m.message == 84 then m.simp_name = '麻痺している'
                elseif m.message == 30 then m.simp_name = '見切り'
                elseif m.message == 31 then m.simp_name = '消費'
                elseif m.message == 32 then m.simp_name = '回避'
                elseif m.message == 67 and (act.category == 1 or act.category == 11) then m.simp_name = 'クリティカル'
                elseif m.message == 106 then m.simp_name = 'ひるんでいる'
                elseif m.message == 153 then m.simp_name = act.action.name .. '失敗'
                elseif m.message == 244 then m.simp_name = 'かすめとる失敗'
                elseif m.message == 282 then m.simp_name = '回避'
                elseif m.message == 373 then m.simp_name = '回復'
                elseif m.message == 352 then m.simp_name = '遠隔攻撃'
                elseif m.message == 353 then m.simp_name = '遠隔クリティカル'
                elseif m.message == 354 then m.simp_name = '遠隔攻撃ミス'
                elseif m.message == 576 then m.simp_name = 'RA hit squarely'
                elseif m.message == 577 then m.simp_name = 'RA struck true'
                elseif m.message == 157 then m.simp_name = '乱れ撃ち'
                elseif m.message == 76 then m.simp_name = '対象が範囲内にいない'
                elseif m.message == 77 then m.simp_name = '散華'
                elseif m.message == 360 then m.simp_name = act.action.name..' (JA再使用可能)'
                elseif m.message == 426 or m.message == 427 then m.simp_name = 'Bust! '..act.action.name
                elseif m.message == 435 or m.message == 436 then m.simp_name = act.action.name..' (JA再使用可能)'
                elseif m.message == 437 or m.message == 438 then m.simp_name = act.action.name..' (JA再使用可能,TP回復)'
                elseif m.message == 439 or m.message == 440 then m.simp_name = act.action.name..' (SP+JA再使用可能,TP/MP回復)'
                elseif T{252,265,268,269,271,272,274,275,379,650,747}:contains(m.message) then m.simp_name = 'MB '..act.action.name
                ...
                elseif m.message == 522 and simplify then
                    targ = targ..' ('..color_it('スタン',color_arr['statuscol'])..')'
                elseif m.message == 416 and simplify then
                    targ = targ..' ('..color_it('魔攻/魔防アップ',color_arr['statuscol'])..')'
                elseif m.message == 1023 and simplify then
                    targ = targ..' ('..color_it('攻撃力/防御力アップ',color_arr['statuscol'])..')'
                elseif m.message == 762 and simplify then
                    targ = targ..' ('..color_it('全ステータスアップ',color_arr['statuscol'])..')'
                ...
                elseif T{158,188,245,324,592,658}:contains(m.message) and simplify then
                    ...
                    m.status = 'ミス' --- This probably doesn't work due to the if a==nil statement below.
                elseif m.message == 653 or m.message == 654 then
                    m.status = color_it('レジストハック',color_arr['statuscol'])
                elseif m.message == 655 or m.message == 656 then
                    m.status = color_it('完全耐性',color_arr['statuscol'])
                elseif m.message == 85 or m.message == 284 then
                    if m.unknown == 2 then
                        m.status = color_it('レジスト!',color_arr['statuscol'])
                    else
                        m.status = color_it('レジスト',color_arr['statuscol'])
                    end
                ...
                elseif T{75,114,156,189,248,283,312,323,336,355,408,422,423,425,659}:contains(m.message) then
                    m.status = color_it('効果なし',color_arr['statuscol']) -- The status code for "No Effect" is 255, so it might actually work without this line
                end
                if m.message == 188 then
                    m.simp_name = m.simp_name..' (ミス)'
            ...
            --        m.simp_name = m.simp_name..' (No Effect)'
                ...
                local has_line_break = string.find(res.action_messages[m.message][language], '${lb}') and true or false
                ...
                local message = prefix..(clean_msg((msg or tostring(m.message))
                    ...
                    :gsub('${swing}', make_condensedamage_number(m.number))
                    :gsub('${gil}',m.param..' gil'), m.message))
                    if m.message == 377 and act.actor_id == Self.id then
                        send_delayed_message:schedule(0.5,color,windower.to_shift_jis(message))
                    else
                        windower.add_to_chat(color, windower.to_shift_jis(message))
                    end
            ...
            if m.has_add_effect and m.add_effect_message ~= 0 and add_effect_valid[act.category] then
                ...
                elseif m.add_effect_message == 603 then m.simp_add_name = '追加効果: トレジャーハンター'
                elseif m.add_effect_message == 605 then m.simp_add_name = '追加効果: デス'
                elseif m.add_effect_message == 776 then m.simp_add_name = '追加効果: 被連携状態'
                else m.simp_add_name = '追加効果'
                end
...
--                    windower.add_to_chat(color, 'act.action==nil : '..m.message..' - '..m.add_effect_message..' - '..msg)
                    ...
                    local msg = windower.to_shift_jis(clean_msg(msg
                    ...
                    :gsub('${swing}',make_condensedamage_number(m.add_effect_number))
            ...
            if m.has_spike_effect and m.spike_effect_message ~= 0 and spike_effect_valid[act.category] then
                ...
                msg = windower.to_shift_jis(clean_msg(msg
                    ...
                    :gsub('${swing}', make_condensedamage_number(m.spike_effect_number))
...
function simplify_message(msg_ID)
    ...
    if simplify and not T{23,64,133,204,210,211,212,213,214,350,442,516,531,557,565,582}:contains(msg_ID) then
        ...
        local Despoil_msg = {[593] = '攻撃力ダウン', [594] = '防御力ダウン', [595] = '魔法攻撃力ダウン', [596] = '魔法防御ダウン', [597] = '回避ダウン', [598] = '命中ダウン', [599] = 'スロウ',}
        ...
        elseif line_steal and fields.item and fields.ability then
            if T{593,594,595,596,597,598,599}:contains(msg_ID) then
                msg = line_steal..''..string.char(0x07)..'追加効果: '..color_it(Despoil_msg[msg_ID],color_arr['statuscol'])
            else
                msg = line_steal
            end
...
function make_condensedamage_number(number)
    if swingnumber and condensedamage and number > 1 then
        return '[x'..number..'] '
    else
        return ''
    end
...
function get_spell(act)
    ...
    local msg_ID
    if act and act.targets[1] and act.targets[1].actions[1] then
    ...
    elseif msg_ID == 105 then
        ...
        if (not msg_ID) or (not res.action_messages[msg_ID]) then
            ...
            elseif T{3,7,11}:contains(act['category']) then
                if abil_ID < 256 then
                    ...
                    if act.actor.type and act.actor.type == 'mob' then
        ...
        if fields.item then
            if T{125,593,594,595,596,597,598,599}:contains(msg_ID) then
                ...
                spell.item = color_it(res.items[effect_val][language], color_arr.itemcol)
                spell.item_id = res.items[effect_val].id
            else
                spell = res.items[abil_ID]
                local item_article = not simplify and add_item_article(spell.id) or ''
                if spell then
                    spell.name = color_it(item_article..spell[language],color_arr.itemcol)
                    spell.item = color_it(item_article..spell[language],color_arr.itemcol)
...
function get_prefix(category, effect, message, unknown, reaction_lookup)
    local prefix = S{1,3,4,6,11,13,14,15}:contains(category) and (bit.band(unknown,1)==1 and 'かばう ' or '')
                    ..(bit.band(unknown,4)==4 and 'マジックバースト ' or '') --Used on Swipe/Lunge MB
                    ..(bit.band(unknown,8)==8 and 'レジストハック ' or '') --Unused? Displayed directly on message
                    ..(showcritws and bit.band(effect,2)==2 and S{1,3,11}:contains(category) and message~=67 and 'クリティカル ' or '') --Unused? Crits have their own message
                    ..(showblocks and reaction_lookup == 4 and 'ブロック ' or '')
                    ..(showguards and reaction_lookup == 2 and 'ガード ' or '')
